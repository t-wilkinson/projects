...
function run(str: string, parser: P.Parser, expects:any = { value: str }) {
  expect(P.parse(str, parser)).toMatchObject(expects)
}

const value = (str: string, parser: P.Parser, value: any = str) => run(str, parser, { value })
