...
class Ball(GameObject):
    def __init__(self):
        super().__init__(
            img=img['ball'], space=ball_space(space), scale=CIRCLE_SCALE)
        self.reset()
    ...
    def update(self):
        ...
        self.space.body.angular_velocity = (
            np.sign(ang_vel) * min(abs(ang_vel), MAX_ANGLE))
    ...
    def decide(self):
        ...
        # setup input layer
        self.neural_network.X = X = [0 for _ in range(4)]
        X[0] = self.x
        X[1] = self.y
        ...
        x1, x2 = empty_bricks[row_index]
        X[2] = x1
        X[3] = x2

        self.neural_network.feed_forward()
        left, right = self.neural_network.decision().T
