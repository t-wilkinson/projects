...
import { linking, extras, routes } from 'Shared/constants'
...
const pages = {
  ComingSoon: React.lazy(() => import('./src/ComingSoon')),
  Home: React.lazy(() => import(/* webpackPrefetch: true */ './src/Home')),
  LandingPage: React.lazy(() => import('./src/LandingPage')),
  Shop: React.lazy(() => import(/* webpackPrefetch: true */ './src/Shop')),
  PrivacyPolicy: React.lazy(() => import('./src/PrivacyPolicy')),
  Products: React.lazy(() =>
    import(/* webpackPrefetch: true */ './src/Products'),
  ),
}
...
export const App = () => {
  ...
  const cacheImages = (images: (string | number)[]): Promise<void>[] => {
    return images.map(async (image) => {
      if (typeof image === 'string') return Image.prefetch(image).then(() => {})
      else
...
const RootDrawerNavigator = () => (
  ...
  >
    ...
    />
    <RootDrawer.Screen name="Shop" component={pages.Shop} />
  </RootDrawer.Navigator>
)
