...
export const Products = ({ query, navigation, routeName }) => {
  ...
  const shopItemsCount = query.data?.shopItemsCount ?? 0
  const listWidth =
  ...
  const totalPages = Math.ceil(shopItemsCount / PAGE_SIZE)
    ...
    (props) => (
      <Product
        ...
        style={{
          ...
          marginRight: marginRight(props.index),
        }}
  ...
  return (
    <Box flex={1}>
      ...
      >
        <Box>
          <Text variant="subheader">
            {routeName} {shopItemsCount > 0 && `(${shopItemsCount})`}
          </Text>
        </Box>
        ...
        <Box flexDirection="row" justifyContent="flex-end" width="100%" mt="lg">
          <TouchableOpacity onPress={() => navigation.openDrawer()}>
            <Box flexDirection="row" alignItems="center">
              ...
              {/* <FiltersCount fontSize={12} routeName={routeName} /> */}
            </Box>
          </TouchableOpacity>
        </Box>
      )}
      ...
      {query.loading === true ? (
        <FlatList
          key={numColumns}
          renderItem={EmptyProduct}
          data={Array(PAGE_SIZE)
            .fill(0)
            .map((_, i) => ({
              index: i.toString(),
              width: productWidth,
              height: productWidth * 1.25,
              marginRight: marginRight(i),
            }))}
...
const EmptyProduct = ({ item }) => {
  return (
    <Box
      ...
      alignItems="stretch"
    >
      <Box height={item.height + 68 + 16} mt="md" bg="light-gray" />
    </Box>
  )
}
...
const ProductInfo = ({ item }) => (
  <Box flexDirection="row" justifyContent="space-between" mt="md" height={68}>
    <Box flex={1}>
