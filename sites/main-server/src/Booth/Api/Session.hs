...
module Booth.Api.Session where

-- Local imports
import Quaalude
import Core.Session (toSession)
import Core.Config (Config, sk)
import Core.Helpers (getMatchingRecordsByField, addUserCookie, baseCookie, stripeReqBS, parseStripeRes, addSessionUserCookie)
...
import Booth.Helpers (isAdmin, getStripeKey)
...
login maybeCookies user@UserLogin{..} = do
    sk' ← asks (^.sk)
    ...
    -- isAdmin' ← isAdmin (E.entityVal entityUser)
    let session = toSession sk' (E.entityKey entityUser) ""
        returnUser = ReturnUser email (toSToken $ sesToken session)
    pure $ if remember
...
register maybeCookies UserRegister{..} = do
    ...
    -- validate info
    case (vName name, vEmail email,vPassword password) of
      (Success _, Success _, Success _) → pure ()
      _ → throwError CE.parsingCredentials

    -- create customer
    body ← stripeReqBS key [] POST ["v1","customers"]
    customerId ← parseStripeRes $ body ^? JSON.key "id"

    -- insert user
    hashed ← hash password
    let user = User name email hashed customerId 0
    key' ← (runDb $ E.insertUnique $ user) >>= maybe (throwError CE.accountExists) pure

    -- return header
    isAdmin' ← isAdmin user
    let session = toSession sk' key' ""
        returnUser = ReturnUser email (toSToken $ sesToken session)
    pure $ if remember && not isAdmin'
              then addUserCookie session maybeCookies returnUser
              else addSessionUserCookie session maybeCookies returnUser
...
logout
    ...
    ⇒ m (Headers '[Header "Set-Cookie" SetCookie] ())
logout = pure $ addHeader cookie ()
  where cookie = (baseCookie userField "")
...
changeUser
    ...
    ⇒ Token
    → m (Headers '[Header "Set-Cookie" SetCookie] ())
changeUser userToken = pure $ addHeader cookie ()
  where
    secondsInAYear = 365 * 24 * 60 * 60
    cookie = (baseCookie userField userToken)
        { setCookieMaxAge = Just $ secondsToDiffTime secondsInAYear
        }
...
vClean ∷ Validate
vClean = Success . T.unpack . T.strip . T.pack
