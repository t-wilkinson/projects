import {ofMap, dispatch, mergeEpics, createStore} from "rxjs-redux-store"
import myslices from "./slice"
import * as api from "./api"
import {
  basicAuth, parseCents, toyyyymmdd, ymdToUTC,
} from "./components"
...
ofMap(["root"], ["changeUser"], (s,a) => {
  api.postSessionUser(
    ...
    (res) => {
      dispatch({filter: "root", type: "getUser"})
      a.history.push("/dashboard" || a.history.location.s)
    },
    (e) => console.error(e.message),
  )
}),
...
ofMap(["timeSelector"], ["post"], (s,a) => {
  ...
  api.postService(
    { description: a.description,
      ...
      mixing: a.mixing, mastering: a.mastering, recording: a.recording
    },
    basicAuth(s.root),
    (s) => {
      a.onSuccess(s)
...
// TODO: requires tracks, and promos
...
ofMap(["dashboard"], ["getCharge"], (s,a) => {
  api.getPaymentCharge(
    basicAuth(s.root),
    (res) => a.onSuccess(parseCents(res)),
    (e) => a.onError(e),
  )
}),
ofMap(["stripe"], ["createPayment"], (s,a) => {
  api.postPaymentIntent(
    basicAuth(s.root),
    (res) => a.onSuccess({
      clientSecret: res.clientSecret,
      charge: parseCents(res.charge),
    }),
    (e) => a.onError(e),
  )
}),
