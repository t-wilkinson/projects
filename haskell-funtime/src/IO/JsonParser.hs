...
charP x = Parser f
 where
  f input@(inputUncons -> Just (y, ys))
    | y == x = Right (ys, x)
    | otherwise = Left $ ParserError
      (inputLoc input)
      ("Expected '" ++ [x] ++ "', but found '" ++ [y] ++ "'")
  f input = Left $ ParserError
    (inputLoc input)
    ("Expected '" ++ [x] ++ "', but reached end of string")

stringP :: String -> Parser String
stringP str = Parser $ \input -> case runParser (traverse charP str) input of
  Left _ -> Left $ ParserError
    (inputLoc input)
...
parseIf desc f = Parser $ \input -> case input of
...
doubleLiteral =
  (\sign int frac expo -> sign * (int + frac) * (10 ** expo))
    <$> (minus <|> pure 1)
    <*> (read <$> digits)
...
escapeUnicode = chr . fst . head . readHex <$> sequenceA
  (replicate 4 (parseIf "hex digit" isHexDigit))
...
main = do
  ...
  case runParser jsonValue $ Input 0 testJsonText of
    ...
    Left (ParserError loc msg) -> do
      putStrLn
        $  "[ERROR] Parser failed at character "
        ++ show loc
        ++ ": "
        ++ msg
      exitFailure
 where
  testJsonText = unlines
    [ "{"
