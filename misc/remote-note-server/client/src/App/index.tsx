...
function App() {
  ...
  const onChange = React.useCallback(
    (event: any) => {
      ...
      filename &&
        api.getFile(state, filename).then(file => {
  ...
  React.useEffect(() => {
    if (!state.privateKey || !state.publicKey) {
      createKeyPair()
      .then(({publicKey, privateKey}) => dispatch({ type: 'create-key-pair', publicKey, privateKey}))
    }
  }, [])
  ...
  React.useEffect(() => {
    api.getFiles(state).then(files => dispatch({ type: 'update-files', files }))
...
const DisplayFile = ({ zettel }) => {
  ...
  const saveContent = () => {
    ...
    if (innerText !== state.files[state.index].body) {
      api.saveFile(state, state.files[state.index].name, innerText)
    }
  }
