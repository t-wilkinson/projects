...
pub mod iptable {
    ...
    // TODO: work with https, does this require replacing dns and creating custom CA or something?
    pub fn create_rule(protocol: Protocol, port: u16) -> Command {
        Command::new(format!("iptables -t nat -I OUTPUT -p tcp ! --sport {protocol:?} --dport 80 -j REDIRECT --to-ports {port}"))
    }

    pub fn remove_rule(protocol: Protocol, port: u16) -> Command {
        Command::new(format!("iptables -t nat -D OUTPUT -p tcp ! --sport {protocol:?} --dport 80 -j REDIRECT --to-ports {port}"))
    }
}
